...
function initializeRevealAnimations() {
  ...
  elements.forEach((element) => {
    ...
    /*
      Elementos que já estão na tela precisam começar visíveis. Isso evita um
      quadro vazio enquanto o navegador agenda o primeiro IntersectionObserver.
    */
  ...
  /*
    A classe que permite ocultar elementos só é adicionada depois que todos os
    observadores estão prontos. Se outro arquivo falhar, o conteúdo permanece
    visível — princípio de melhoria progressiva.
  */
  document.body.classList.add("js-enabled");

  /* Última proteção: nunca manter texto invisível por falha do navegador. */
  window.setTimeout(() => {
    elements.forEach((element) => element.classList.add("is-visible"));
  }, 1800);
}
...
/*
  No GitHub Pages, nomes de arquivo diferenciam maiúsculas de minúsculas. Se o
  data.js não for publicado com esse nome exato, preservamos o conteúdo fixo e
  mostramos uma mensagem clara no acervo em vez de esconder a página inteira.
*/
if (data?.timeline && data?.editions) {
  renderTimeline();
  buildEditionTabs();
  renderEdition(state.activeYear);
} else {
  console.error("SENEL: o arquivo data.js não foi carregado.");
  editionTabs.replaceChildren(
    createElement("p", "empty-state", "Não foi possível carregar os dados das edições. Verifique se o arquivo data.js foi publicado na raiz do site."),
  );
  $("#edition-panel").hidden = true;
}
